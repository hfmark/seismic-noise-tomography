...
for date in dates:
    ...
    def preprocessed_trace(trace_resp):
        """
    ...
    if MULTIPROCESSING['cross-corr']:
        ...
        def xcorr_func(pair):
            ...
            if abs(len(tr1.data) - len(tr2.data))%2 != 0:  # odd number length difference
                # pick one of the traces and lop off a sample to make the difference even
                tr1.data = tr1.data[:-1]
