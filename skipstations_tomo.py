...
SKIP_STATIONS = ['AY01','LL02','GRAF','GUMN','CTS01','AMG01','LSR01','OHRS']
...
for pickle_file in pickle_files:
    ...
    period = PERIODS[0]
    for sta in SKIP_STATIONS:
        print("\nDoing station {} ".format(sta))
        ...
        for passnb in range(npass):
            ...
            title = ("{0}: {1} s, {2} pass, grid {3} x {3} deg, "
                     "min SNR = {4}, corr. length = {5} km, alpha = {6}, "
                     "beta = {7}, lambda = {8} ({9} paths, {10} rejected)")
            title = title.format(sta, period, fancy_names[passnb],
                                 GRID_STEPS[passnb], MINSPECTSNRS[passnb],
                                 CORR_LENGTHS[passnb], ALPHAS[passnb],
            ...
            # figure
            title = ("{0}: {1} s, one pass, grid {2} x {2} deg, "
                     "min SNR = {3}, corr. length = {4} km, alpha = {5}, "
                     "beta = {6}, lambda = {7} ({8} paths)")
            title = title.format(sta, period, GRID_STEPS[-1], MINSPECTSNRS[-1],
